...
`Ctrl-C` 会先停止发送新目标，待当前 SDK 调用返回后再由主流程发送一次 xArm stop；不要连续猛按。若机器人仍在运动、终端卡住或状态不明确，实体急停或 xArm Studio Stop 优先于软件停止。确认机械臂停止后，可在另一终端用 `pgrep -af teleop_six_axis_gated.py` 找到卡住的进程，再执行 `kill -KILL <PID>`。
...
实机持续六轴测试已经验证了完整六轴目标流和 Ctrl-C 后回 P0 的安全流程，但它仍是 Phase-A 的复用方案：大范围时会出现姿态偏差，且 leader 的轴线/连杆/支架与 xArm6 不同，会产生副轴编码器随动。当前脚本每周期以一个完整的 `set_servo_angle_j` 六轴目标下发，并非按轴依次发送；若感觉各轴依次运动，应优先检查机械耦合、每轴映射比例、限速和 xArm 本体动态差异。

50 Hz 下，leader 读取约 2 ms、xArm SDK 发送均值约 1.5 ms；可感知的“慢”主要来自 20 ms 控制周期和 `safe` 档的 0.20 rad/s 限速，偶发的控制箱发送长尾会额外造成瞬时延迟。迁移到 C++ 可能改善宿主端调度，但不能修复运动学/机械不匹配；下一优先级是大行程多点标定与六轴时间序列诊断。
