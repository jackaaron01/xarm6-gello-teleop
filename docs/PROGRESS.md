...
- J3：原始样本 `+115 raw / +0.3414 rad`，重复样本 `+160 raw / +0.3414 rad`；方向一致，取中位数比例。
...
- J3/J5：尚未进行实机单轴 teleop；先完成只读映射预览。
...
J3 预览通过后，以相同命令只移动 leader J5，检查主导轴 `wrist_2` 及方向。之后再决定是否为 J3/J5 开放单轴实机测试。
